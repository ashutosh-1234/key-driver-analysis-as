...
def show_page() -> None:
    ...
    # 1 ▸ prepare data (also refresh selections)
...
def prepare_regression_data() -> None:
    """Pull everything needed for modelling and refresh selection lists."""
    ...
    if "sel_factored" in st.session_state:
        st.session_state.sel_factored = [v for v in st.session_state.sel_factored
                                         if v in st.session_state.factor_names]
    else:
        st.session_state.sel_factored = st.session_state.factor_names.copy()

    if "sel_raw" in st.session_state:
        st.session_state.sel_raw = [v for v in st.session_state.sel_raw
                                    if v in st.session_state.raw_features]
    else:
        st.session_state.sel_raw = []

    # Debug
    st.info(
        f"Factors: {len(st.session_state.factor_names)} · "
        f"Raw pool: {len(st.session_state.raw_features)} · "
        f"Selected target: {target_col}"
    )
...
def build_current_X() -> pd.DataFrame:
    """Return dataframe with currently-selected variables, dropping any stale names."""
    sel_f = [v for v in st.session_state.sel_factored
             if v in st.session_state.factor_names]
    sel_r = [v for v in st.session_state.sel_raw
             if v in st.session_state.raw_features]
...
def get_clean_X_y() -> tuple[pd.DataFrame, pd.Series]:
    """Return X, y after dropping any rows with NA so their lengths match."""
    X = build_current_X()
    y = st.session_state.y_target
    combined = pd.concat([X, y], axis=1).dropna(axis=0)
...
def train_and_evaluate_model() -> None:
    ...
    if X.empty:
        ...
        return
    if len(X) < 10:
        st.error("Not enough rows after cleaning. Need at least 10.")
        return
    if y.nunique() < 2:
    ...
    stratify_opt = y if min_class >= 3 else None

    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=0.30, random_state=42, stratify=stratify_opt
    )
